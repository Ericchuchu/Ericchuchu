...
working_rules:
  ...
  - transaction costs belong in the first backtest, not the last
```
...
| **Cross-sectional alpha** | Point-in-time factor research, return ranking, regime-aware risk scaling | Factor construction with dynamic universe masking and per-date neutralization against size and sector; walk-forward gradient-boosting rankers tuned with Optuna; hidden Markov regime models; distributional-distance and options-positioning features; Gaussian-process regression; formulaic-alpha search with expression trees and a Transformer generator; portfolios under turnover and position limits |
| **Reinforcement learning** | Hierarchical agents for minute-level trading | MacroHFT-based agents, Rainbow DQN and QR-DQN benchmarks, multi-scale temporal state encoders, evaluation tooling |
| **Market microstructure** | Exchange simulation and short-horizon signals | Asynchronous exchange simulator, matching against live order-book snapshots, maker and taker fees, latency injection, order-book imbalance features |
...
|---|---|---|
| [`Constrained-Deep-Learning-for-Option-Pricing`](https://github.com/Ericchuchu/Constrained-Deep-Learning-for-Option-Pricing) | Undergraduate thesis. One-day-ahead pricing of TAIEX index options; a dual-branch ConvLSTM + Transformer network with a PDE-style residual penalty; stored test MSE is 48.7% below the ConvLSTM baseline. The README states what each constraint does and does not enforce, and the known weaknesses of the evaluation | PyTorch · ConvLSTM · MultiPatchFormer · FANformer · Kou / FFT benchmark |
| [`MacroHFT`](https://github.com/Ericchuchu/MacroHFT) (team project, fork) | Hierarchical reinforcement learning for minute-level ETHUSDT trading, extending MacroHFT (KDD 2024) with a regime-aware Dynamic Hybrid coordinator. My part: baseline strategies and evaluation modules | PyTorch · DQN variants · expert mixing |
| [`async-exchange-simulator`](https://github.com/Ericchuchu/async-exchange-simulator) | `asyncio` paper exchange that relays a live crypto-exchange feed and matches orders against the live order book: maker and taker fees, hedge-mode positions, live P&L monitor, offline tests. Built in a five-person trading training program; the README lists the modelling limits and what I did not write | asyncio · WebSockets · matching engine · unittest |
